 ...
 * hand-measured findings in this sweep, and for four of the findings in this
 * repo: the exhibit-header disclosure control at 1.21:1 against the page, the
 * BW₃₂ message input at 1.42:1, the back-to-top button at 1.55:1, and the
 * shared top bar's `.cl-btn` at 1.49:1 in the light theme. The repo's previous
 * 1.4.11 check (`e2e/border.spec.ts`, now deleted) measured `.param-select`
 * and nothing else — which is the ONE selector `var(--control-border)` was
 * already applied to, so it could not fail.
 *
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * is unmeasurable and paints nothing — which is the only branch this page
     * takes today, since `.param-select` paints its chevron from an inline
     * `url("data:image/svg+xml,…")` and nothing here uses a gradient. Shared
     ...
     *
     * A driven pass here walks twelve exhibits, and — the expensive part — the
     * 96-cell LWE matrix, three 32-cell BW₃₂ heat grids and two 8x8
     * lattice-structure grids, all rendered one `<div>` per cell, so hundreds
     * of sibling elements re-walk the same ancestors up to `<body>`. Without
     ...
     * gradients; the walk therefore found nothing opaque, fell through to
     * WHITE, and reported a bright cyan disclosure triangle at 3.95:1 against a
     * page that is actually near-black. The tell was that the number came out
     * IDENTICAL in both themes — a ratio that cannot depend on the theme is not
     * measuring the theme. This page sets an opaque `background-color` on both
     * `html` and `body`, so the first branch answers immediately; the sampler
     * is borrowed from `contrast.ts` wholesale rather than approximated a
     * second time, so the two oracles cannot disagree.
     */
