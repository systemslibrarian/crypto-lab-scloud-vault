 ...
 *    an unknown backdrop, or a `color-mix()`. This page has both kinds. The
 *    hero aside is `background: color-mix(in oklab, var(--accent) 6%,
 *    transparent)`, and the guided walkthrough's progress dots, the TOC level
 *    pills and the `.toc-badge` are translucent literal tints
 *    (`rgba(63,185,80,.2)` and friends) laid over a card that is itself over
 *    the page. axe files every one of those under `incomplete`, which is not
 *    the array a violations-only gate reads — and one of them, the `.done`
 *    progress dot, was a real 4.03:1 failure in the light theme.
 ...
 *    at `.85` and `.btn:disabled` at `.5`, the latter live in this drive
 *    because the ternary shuffle disables both of its buttons for the whole of
 *    its animated run.
 *
 ...
 *    boxes are `.matrix-scroll` and `.table-scroll` (`overflow-x: auto` around
 *    a 96-cell matrix and a twelve-row KEM comparison), and at phone width the
 *    `.comparison-table` itself, which becomes `display: block; overflow-x:
 *    auto`. Content scrolled past those is not dimmed or partly drawn — it is
 ...
 *    draws every figure it has — the BW₃₂ heat grids, the bar charts, the
 *    lattice-structure grids — in HTML and CSS grid rather than SVG, so the
 ...
 * rule. `nontext.ts` measures those separately; this page has exactly one that
 * carries meaning, `.fo-branch-card.dim::after { content: 'not taken' }`, which
 * is the non-colour cue marking which branch of the FO transform was NOT taken
 * and is driven and measured by the gate in both outcomes.
 *
 * SECOND, `aria-hidden` subtrees, which this walk skips to match axe's own
 * boundary. What this lab hides that way was enumerated rather than assumed —
 * see the note on `ariaHidden` below, and the hand measurement of the BW₃₂
 * grid's plus/minus glyphs in `src/exhibits/exhibit3.ts`, which are inside a
 * `role="img"` and are therefore the one place on this page where a meaningful
 * glyph is invisible to BOTH oracles.
 */
 ...
 * uses. The genuinely short-lived styling here is `.fade-in`, which this lab
 * puts on every result box it generates, and `.guide-step`, which it puts on
 * every step of the guided walkthrough — both run `@keyframes fadeIn` from
 * `opacity: 0`. Under the reduced motion this gate asserts, the stylesheet
 * clamps the duration to 0.01ms, so the animation is over before a scan can
 * begin and there is no half-faded frame to catch.
 *
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * moment a gradient runs the height of a document several screens tall:
     * text near the top then sits on a different surface than text near the
     ...
     * (`url()`, `none`) is unmeasurable and paints nothing — which is the only
     * background-image branch this page currently takes, since `.param-select`
     * paints its chevron from an inline `url("data:image/svg+xml,…")` and
     * nothing here uses a gradient at all. The gradient branches are kept
     * because a flat `background-color` page is one commit away from a tinted
     * one, and the failure mode of not having them is a ratio measured against
     * a surface the page does not paint.
     */
     ...
     * that is `.matrix-scroll` around the 96-cell LWE matrix, `.table-scroll`
     * around the KEM comparison and the plain-English comparison, `.formula`,
     * and — at phone width only — `.comparison-table` itself, which the 720px
     * media query turns into `display: block; overflow-x: auto`.
     */
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // exactly the shape that guard exists for. This lab's only SVG is the
      // shared header's hamburger and GitHub marks, both `aria-hidden` and
      // glyph-free, and the hamburger is three stroke-only <line>s — precisely
      // the case the FILLED test exists for. The rule is the same one that
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // `<details>` element. This lab has NONE — every disclosure it has is a
      // custom `.exhibit-header` `role="button"` toggling an inline
      // `style.display`, which `display: none` already covers — and `boot()`
      // asserts `details` count is 0 so that claim cannot rot. The check stays
      // because it costs nothing and because the gate this replaces set
      // `d.open = true` on every `<details>` it could find, which is the
      // habit that makes the first one added here get force-revealed.
      ...
      // page uses it TWICE: the shared header's `.cl-skip-link` parks at
      // `top: -3rem` and the lab's own `.skip-link` at `top: -100%`, and each
      // slides in only on focus. Measuring a parked copy invents a failure for
      ...
      //
      // This lab does NOT currently use the idiom — `style.css` defines no
      // `.sr-only` and nothing here sets a zero-area `clip` — and the guard is
      // kept anyway rather than deleted as unreachable, because the idiom is
      // the standard way to add a visually-hidden label and the failure mode of
      // not having it is silent and inverted: an element clipped to nothing
      // still has a rect and still passes `checkVisibility()`, so the walk
      // measures ink that was never laid down and INVENTS a failure (1.15:1 for
      // text nobody can see, elsewhere in this fleet). It is deliberately
     ...
     * the `.exhibit-number` badge that repeats the number already in the TOC
     * link beside it, the `.toggle-icon` (the ▶/▼ whose state is carried by the
     * header's `aria-expanded`), `#reading-progress`, the shared header's two
     * SVG marks, and the `.bw32-cell` divs inside the three `role="img"` heat
     * grids. Only the last of those carries a glyph rather than a repeat of
     * adjacent text, and it is exactly the shared blind spot this note exists
     * to name — so it was measured by hand and fixed in
     * `src/exhibits/exhibit3.ts` (2.54:1 white on `--positive`, now 7.45:1 with
     * `--on-fill`), not left to an oracle that cannot reach it.
     *
     * Nothing on this page hides a VALUE. No generated key, ciphertext, shared
     * secret, verdict or measured timing is inside an `aria-hidden` subtree —
     * which was checked rather than assumed, because that is where both oracles
     * stop looking and it is the one place a live readout can hide from a whole
     * accessibility gate.
     */
     ...
     *
     * This lab has no SVG of its own — every figure it draws (the BW₃₂ heat
     * grids, the bar charts, the lattice-structure grids) is HTML and CSS grid.
     * The guard stays because the usual way the mistake arrives is exactly how
     * this lab builds its DOM: `innerHTML` template literals, in twelve
     * separate exhibit modules. Interpolating a string ARRAY into one makes JS
     ...
     * is twelve exhibits and many screens deep, and `style.css` already sets
     * `body { min-height: 100vh }` one line away from the `height: 100%` that
     * breaks it. The day that lands, everything below the first screen would be
     * measured against a white page that does not exist — in a lab whose dark
